...
import { Maximize2, ZoomIn, ZoomOut, Eye, Brain, Table, FileText, AlertTriangle, CheckCircle2, GitMerge, List, MapPin } from 'lucide-react';
...
const HeatMirror: React.FC<HeatMirrorProps> = ({ heatmap, entities }) => {
  ...
  // Active entity is either hovered or selected
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-lg flex flex-col h-full overflow-hidden">
      {/* Toolbar */}
      <div className="h-12 border-b border-slate-800 bg-slate-900/80 px-4 flex items-center justify-between z-10 shrink-0">
        ...
        <div className="flex items-center gap-2">
            <button onClick={() => setShowList(!showList)} className={`p-2 rounded transition-colors ${showList ? 'bg-slate-800 text-white' : 'text-slate-400 hover:text-white'}`} title="Toggle Entity List">
                <List size={16} />
            </button>
      ...
      <div className="flex-1 flex overflow-hidden">
                    ...
                    return (
                        <div
                            ...
                            style={{
                                ...
                                opacity: activeEntityId && !isActive ? 0.3 : 1
                            }}
                        >
                            {/* Status Pin */}
                            <div className={`absolute -top-3 -right-3 w-6 h-6 rounded-full flex items-center justify-center text-white border-2 bg-slate-900 ${borderColor} transition-transform ${isActive ? 'scale-110' : 'scale-90'}`}>
                                ...
                                {isDiverged && (
                                    <div className="mt-3 bg-rose-950/50 border border-rose-500/30 p-2 rounded">
                                        ...
                                        <div className="text-[10px] text-rose-300">
                                            Models disagree on value. Human review recommended.
                                        </div>
                                    </div>
                                )}
                            </div>
          ...
          {/* Interactive List Sidebar */}
          <div className={`w-64 bg-slate-900 border-l border-slate-800 transition-all duration-300 flex flex-col ${showList ? 'mr-0' : '-mr-64'}`}>
              <div className="p-3 border-b border-slate-800 flex items-center justify-between">
                  <span className="text-xs font-bold font-mono text-slate-400 uppercase">Extracted Entities</span>
                  <span className="text-[10px] bg-slate-800 px-1.5 py-0.5 rounded text-slate-500">{entities.length}</span>
              </div>
                  ...
                  {entities.map(entity => (
                      ...
                      >
                          <div className="flex justify-between items-start mb-1">
                              <span className={`text-[10px] font-mono font-bold uppercase ${activeEntityId === entity.id ? 'text-white' : 'text-slate-400'}`}>
                                  {entity.name}
                              </span>
                              {entity.status === 'diverged' && <AlertTriangle size={10} className="text-rose-500" />}
                              {entity.memoryHit && <Brain size={10} className="text-fuchsia-500" />}
                          </div>
                          <div className="text-xs font-mono text-slate-300 truncate">
                              {entity.value} <span className="text-slate-500">{entity.unit}</span>
                          </div>
                      </div>
                  ))}
              </div>
